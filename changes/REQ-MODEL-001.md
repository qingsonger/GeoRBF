...
diagnostics. CPD models additionally retain the original polynomial-action
matrix and provenance, complete RRQR/SVD rank decision, verified orthonormal
null-space and quality evidence, and the deterministic projected-energy
matrix. It retains no semantic builder, canonical problem, dense matrix,
...
derivatives without finite differences. The global-anisotropy API now states
explicitly that its points and returned pre-transform derivatives belong to
the caller's current coordinate system; fitted-model external
original-coordinate outputs remain the separate normalization chain-rule
boundary.
...
coefficients, capabilities, general diagnostics, and complete CPD assembly
evidence in deterministic order. This is a borrowed input view for the later
schema requirement; it does not declare enum discriminants, debug text, or
Rust memory layout as a wire format and adds no reader, writer, checksum, or
migration behavior.
...
polynomial coefficients and Hessian, deterministic complete CPD rank,
four-by-one null-space, quality, and nonempty projected-energy evidence,
Matérn center-capability boundaries, builder independence, deterministic
record ordering, compile-time `Send + Sync`, and concurrent bit-identical
reads. Rustdoc, a runnable example, a deterministic twelve-center
D=1/D=2/D=3 evaluation benchmark, three-platform benchmark smoke routing,
